...
trait BuilderTrait
{
     ...
     * @return SlimPlugin
     */
    private function init_register_hooks(\CPWFreeVendor\WPDesk_Plugin_Info $plugin_info, \CPWFreeVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
        if ($plugin instanceof \CPWFreeVendor\WPDesk\PluginBuilder\Plugin\Activateable) {
            \register_activation_hook($plugin_info->get_plugin_file_name(), [$plugin, 'activate']);
        }
        if ($plugin instanceof \CPWFreeVendor\WPDesk\PluginBuilder\Plugin\Deactivateable) {
     ...
     * @param SlimPlugin $plugin
     */
    private function store_plugin(\CPWFreeVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
        $storageFactory = new \CPWFreeVendor\WPDesk\PluginBuilder\Storage\StorageFactory();
     ...
     * @param SlimPlugin $plugin
     */
    private function init_plugin(\CPWFreeVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
