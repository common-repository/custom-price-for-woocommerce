 ...
 */
class WPDeskLoggerFactory extends \CPWFreeVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\CPWFreeVendor\Monolog\Registry::hasLogger($name)) {
            \CPWFreeVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\CPWFreeVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \CPWFreeVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \CPWFreeVendor\Monolog\Logger($name);
        }
        if (\CPWFreeVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \CPWFreeVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\CPWFreeVendor\Monolog\Logger $logger)
    {
        if (!\defined('CPWFreeVendor\\WC_LOG_THRESHOLD')) {
            \define('CPWFreeVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \CPWFreeVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \CPWFreeVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \CPWFreeVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \CPWFreeVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\CPWFreeVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \CPWFreeVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \CPWFreeVendor\Monolog\Registry::hasLogger($name);
    }
}
