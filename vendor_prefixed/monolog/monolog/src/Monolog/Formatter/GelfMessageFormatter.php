...
class GelfMessageFormatter extends \CPWFreeVendor\Monolog\Formatter\NormalizerFormatter
{
    ...
    {
        if (!\class_exists(\CPWFreeVendor\Gelf\Message::class)) {
        ...
        if (\method_exists(\CPWFreeVendor\Gelf\Message::class, 'setFacility')) {
            $this->gelfVersion = 1;
        }
    }
    /**
     * {@inheritDoc}
     */
    public function format(array $record) : \CPWFreeVendor\Gelf\Message
    {
        $context = $extra = [];
        ...
        if ($len > $this->maxLength) {
            $message->setShortMessage(\CPWFreeVendor\Monolog\Utils::substr($record['message'], 0, $this->maxLength));
        }
        ...
        foreach ($extra as $key => $val) {
            ...
            if ($len > $this->maxLength) {
                $message->setAdditional($this->extraPrefix . $key, \CPWFreeVendor\Monolog\Utils::substr((string) $val, 0, $this->maxLength));
                continue;
            }
        ...
        foreach ($context as $key => $val) {
            ...
            if ($len > $this->maxLength) {
                $message->setAdditional($this->contextPrefix . $key, \CPWFreeVendor\Monolog\Utils::substr((string) $val, 0, $this->maxLength));
                continue;
            }
