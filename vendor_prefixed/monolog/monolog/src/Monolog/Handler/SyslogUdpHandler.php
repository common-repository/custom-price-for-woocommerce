...
class SyslogUdpHandler extends \CPWFreeVendor\Monolog\Handler\AbstractSyslogHandler
{
     ...
     */
    public function __construct(string $host, int $port = 514, $facility = \LOG_USER, $level = \CPWFreeVendor\Monolog\Logger::DEBUG, bool $bubble = \true, string $ident = 'php', int $rfc = self::RFC5424)
    {
        if (!\extension_loaded('sockets')) {
            throw new \CPWFreeVendor\Monolog\Handler\MissingExtensionException('The sockets extension is required to use the SyslogUdpHandler');
        }
        parent::__construct($facility, $level, $bubble);
        $this->ident = $ident;
        $this->rfc = $rfc;
        $this->socket = new \CPWFreeVendor\Monolog\Handler\SyslogUdp\UdpSocket($host, $port);
    }
    ...
    private function splitMessageIntoLines($message) : array
    {
        ...
        if (\false === $lines) {
            ...
            throw new \RuntimeException('Could not preg_split: ' . $pcreErrorCode . ' / ' . \CPWFreeVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $lines;
    }
    /**
     ...
     */
    public function setSocket(\CPWFreeVendor\Monolog\Handler\SyslogUdp\UdpSocket $socket) : self
    {
        $this->socket = $socket;
        return $this;
    }
