...
class BrowserConsoleHandler extends \CPWFreeVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \CPWFreeVendor\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \CPWFreeVendor\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \CPWFreeVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
