...
class ChromePHPHandler extends \CPWFreeVendor\Monolog\Handler\AbstractProcessingHandler
{
    ...
    protected function send() : void
    {
        ...
        $json = \CPWFreeVendor\Monolog\Utils::jsonEncode(self::$json, \CPWFreeVendor\Monolog\Utils::DEFAULT_JSON_FLAGS & ~\JSON_UNESCAPED_UNICODE, \true);
        $data = \base64_encode($json);
        if (\strlen($data) > 3 * 1024) {
            self::$overflowed = \true;
            $record = ['message' => 'Incomplete logs, chrome header size limit reached', 'context' => [], 'level' => \CPWFreeVendor\Monolog\Logger::WARNING, 'level_name' => \CPWFreeVendor\Monolog\Logger::getLevelName(\CPWFreeVendor\Monolog\Logger::WARNING), 'channel' => 'monolog', 'datetime' => new \DateTimeImmutable(), 'extra' => []];
