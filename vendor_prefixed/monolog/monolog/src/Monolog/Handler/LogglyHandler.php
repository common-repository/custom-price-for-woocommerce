...
class LogglyHandler extends \CPWFreeVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $token, $level = \CPWFreeVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('curl')) {
            throw new \CPWFreeVendor\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \CPWFreeVendor\Monolog\Handler\Curl\Util::execute($ch, 5, \false);
    }
